...
def getList():
    ...
    return render_template('nice.html', **locals())
...
def user():
    ...
    u = [[request.form['server'], request.form['port'], request.form['password'], request.form['method'], request.form['protocol'], request.form['remarks']]]
    print (u)
    execute_values(User().cursor, "INSERT INTO hope(server, port, password, method, protocol, remarks) VALUES %s", u)
    User().cursor.close()
    User().conn.close()
